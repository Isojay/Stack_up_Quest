...
impl<T: Into<f64> + Copy> Shape<T> {
    fn area(&self) -> f64 {
        match self {
            Shape::Circle(diameter) => PI * ((*diameter).into() / 2.0).powi(2),
            Shape::Triangle(base, height) => (*base).into() * (*height).into() / 2.0,
...
fn main() {
    ...
    println!("The area of the triangle with a base of {} and a height of {} is {:.5}",
        base, height, triangle_area);
    println!("The area of the rectangle with a width of {} and a length of {} is {:.5}",
        width, length, rectangular_area);
    println!("The area of the circle with a diameter of {} is {:.5}",
